...
----------------------------------------------------------------------
-- DROP/CREATES sequence_values TABLE
...
-- CREATES FUNCTION THAT GENERATES THE ID'S 
-- IN sequence_values TABLE
...
CREATE TRIGGER temperature_sensor_id_trigger
BEFORE INSERT
  ON temperature_sensor FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;

-- HUMIDITY SENSOR TRIGGER
DROP TRIGGER IF EXISTS humidity_sensor_id_trigger ^;
CREATE TRIGGER humidity_sensor_id_trigger
BEFORE INSERT
  ON humidity_sensor FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;

-- TEMPERATURE_HUMIDITY SENSOR TRIGGER
DROP TRIGGER IF EXISTS temperature_humidity_sensor_id_trigger ^;
CREATE TRIGGER temperature_humidity_sensor_id_trigger
BEFORE INSERT
  ON temperature_humidity_sensor FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;

-- PROXIMITY SENSOR TRIGGER
DROP TRIGGER IF EXISTS proximity_sensor_id_trigger ^;
CREATE TRIGGER proximity_sensor_id_trigger
BEFORE INSERT
  ON proximity_sensor FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;

-- MOISTURE SENSOR TRIGGER
DROP TRIGGER IF EXISTS moisture_sensor_id_trigger ^;
CREATE TRIGGER moisture_sensor_id_trigger
BEFORE INSERT
  ON moisture_sensor FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;

-- RELAY TRIGGER
DROP TRIGGER IF EXISTS relay_id_trigger ^;
CREATE TRIGGER relay_id_trigger
BEFORE INSERT
  ON relay FOR EACH ROW
BEGIN
  SET NEW.id = sequence_nextval();
END ^;
